    ...
    <section className="py-20 px-4 max-w-6xl mx-auto overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {services.map((service, index) => (
          <div
            key={index}
            className="group bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-100 text-center transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:border-yellow-500 cursor-pointer flex flex-col items-center justify-center min-h-[160px]"
          >
            <div className="text-slate-400 group-hover:text-yellow-500 transition-colors duration-300 mb-4 bg-slate-50 p-4 rounded-full group-hover:bg-yellow-50">
              {service.icon}
            </div>
            <h3 className="font-bold text-slate-800 text-lg leading-tight group-hover:text-slate-900">
              {service.title}
            </h3>
          </div>
        ))}
